...
{	
	...
	public static function incompatible_plugins($incompatible_plugins) {
		...
		if (!empty($plugin_list)) {
			$exit_msg = sprintf( __( '%1$s has detected that there are some active plugins that may be incompatible with it. We recommend disabling the following plugins:', VIDCURA_TXTDOMAIN)
				, '<strong>'.VidCura::name.'</strong>' );
			$exit_msg .= '<ul>';
			foreach ($plugin_list as $p ) {
				$exit_msg .= '<li><strong>'. $p . '</strong></li>';
			}
			$exit_msg .= '</ul>';
			$exit_msg .= sprintf( __('Continued use of these plugins may cause erratic behavior and certain functionality on your site may break entirely. See the <a href="http://code.google.com/p/wordpress-custom-content-type-manager/wiki/IncompatiblePlugins">wiki</a> for more information.', VIDCURA_TXTDOMAIN) );
		}
		
		if ( !empty($exit_msg) ) {
			VidCura::$warnings[] = $exit_msg;
		}
	}
	...
	public static function mysql_version_gt($ver) {
		...
		{	
			$exit_msg = sprintf( __( '%1$s requires MySQL %2$s or newer.', VIDCURA_TXTDOMAIN)
			, VidCura::name, $ver );
			$exit_msg .= ' ';
			$exit_msg .= __('Talk to your system administrator about upgrading.', VIDCURA_TXTDOMAIN);	

			VidCura::$errors[] = $exit_msg;
		}
	}
		...
		foreach ( $required_plugins as $name => $version ) {
			if ( isset($all_plugins_reindexed[$name]) ) {
				if ( !empty($all_plugins_reindexed[$name]['Version']) )
				{
					...
					{
						VidCura::$errors[] = sprintf( __('%1$s requires version %$2% of the %3$s plugin.', VIDCURA_TXTDOMAIN )
							, VidCura::name
							, $version
							, $name );			
					}
				...
				$msg = sprintf( __('%1$s requires version %$2% of the %3$s plugin.', VIDCURA_TXTDOMAIN )
							, VidCura::name
							, $version
							, $name );
				 ...
				 $msg .= ' ';
				 $msg .=  sprintf( 
					__('The %1$s plugin is not installed.', VIDCURA_TXTDOMAIN)
					, $name
				);
				VidCura::$errors[] = $msg;
			}
	...
	public static function wp_version_gt($ver) {
		...
		if (version_compare($wp_version,$ver,'<')) {
			VidCura::$errors[] = sprintf( __('%1$s requires WordPress %2$s or newer. <a href="http://codex.wordpress.org/Upgrading_WordPress">Please update!</a>', VIDCURA_TXTDOMAIN)
			, VidCura::name
			, $ver );
		}
	...
	public static function php_version_gt($ver) {
		...
		if ( version_compare( phpversion(), $ver, '<') ) {
			$exit_msg = sprintf( __('%1$s requires PHP %2$s or newer', VIDCURA_TXTDOMAIN )
				,  VidCura::name
				, $ver );
			$exit_msg .= __('Talk to your system administrator about upgrading.', VIDCURA_TXTDOMAIN);	
			VidCura::$errors[] = $exit_msg;
	...
	public static function php_extensions($required_extensions) {
		...
		foreach ( $required_extensions as $req ) {
			if ( !in_array($req, $loaded ) ) {
				$msg =  sprintf( __('%1$s requires the %2$s PHP extension.', VIDCURA_TXTDOMAIN)
					, VidCura::name
					, $req
				);
				
				$msg .= __('Talk to your system administrator about reconfiguring PHP.', VIDCURA_TXTDOMAIN);
				VidCura::$errors[] = $msg;
			}
		}
